...
def FixRomanOutput(Strng,Target):
    ...
    # print(VowelIU, '***', TargetCons, '***', ConsH, '***', UnAspCons)
    # कि कइ - ki -> k_i
    ...
    if Target == "Custom":
        ConsonantList = '|'.join(GM.CrunchSymbols(GM.Consonants,Target)).replace('^','\^').replace('.','\.')
        VowelSignList = '|'.join(GM.CrunchSymbols(GM.VowelSigns,Target)[1:]).replace('^','\^').replace('.','\.') # OR of all Consonants - k|kh etc
        # Strng = re.sub(r'(i)'+'('+ConsonantList+')'+'('+Schwa+')'+'([^a-zA-Z])', r'\1\2o\4', Strng)
        Strng = re.sub(r'(a|e|o|i|u)'+'('+ConsonantList+')'+'('+Schwa+')', r'\1\2', Strng)
        Strng = re.sub('('+Schwa+')'+'(.)'+'('+Schwa+')'+'([^a-zA-Z])', r'\1\2\4', Strng)
    #     # pass
